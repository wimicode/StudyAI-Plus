...
type GenerateRequest = {
  ...
  exam?: { durationMinutes: number; instructions?: string };
};
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    // Récupérer le cours et ses sources
    ...
    if (body.flashcards) {
      ...
      updates.flashcards = await generateFlashcards(
    ...
    if (body.quiz) {
      ...
      updates.quiz_questions = await generateQuiz(
        combinedContent, course.subject, course.level, course.language,
    ...
    if (body.exam) {
      const { durationMinutes, instructions } = body.exam;
      updates.exam_content = await generateExam(
        combinedContent, course.subject, course.level, course.language,
    ...
    const { error: updateError } = await supabase
      .from('courses').update(updates).eq('id', courseId).eq('user_id', user.id);
    if (updateError) throw updateError;

    return NextResponse.json({ success: true, generated: Object.keys(updates) });
  } catch (error) {
